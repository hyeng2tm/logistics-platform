...
import { Save, Plus, Trash2, Bell, CheckCircle } from 'lucide-react';
        ...
        <Card title={formData.id ? t('system.edit_message', '메시지 수정') : t('system.create_message', '새 메시지 생성')} className="split-view-right" headerActions={
          Object.keys(formData).length > 0 && (
             <div className="message-mgmt-actions">
                {formData.id && (
                  <button className="btn btn-outline message-mgmt-delete-btn" onClick={handleDelete} title={t('common.delete')}><Trash2 size={16} /> {t('common.delete')}</button>
                )}
