...
import { Save, Plus, Eraser } from 'lucide-react';
...
const MenuManagement: React.FC = () => {
  ...
  const handleSave = () => {
    // 1. Validation for Menu Key (i18n Key)
    if (!formData.menuKey) {
        showAlert({ title: t('common.input_error', '입력 오류'), message: t('menu.key_required', '메뉴 키(i18n Key)를 입력해주세요.') });
        return;
    }
    
    // 2. Validation for Menu Name (at least Korean translation)
    if (!formData.translations?.ko) {
        showAlert({ title: t('common.input_error', '입력 오류'), message: t('menu.name_ko_required', '메뉴명(한국어)을 입력해주세요.') });
        return;
    }
  ...
  const getDisplayTitle = (menu: MenuData) => {
    ...
    const displayTitle = (menu.translations && menu.translations[baseLang]) || menu.menuKey;
        ...
        <Card title={formData.id ? t('menu.edit_detail', '메뉴 상세 수정') : Object.keys(formData).length > 0 ? t('menu.create_new', '신규 메뉴 생성') : t('menu.detail_settings', '메뉴 상세 설정')} className="split-view-right" headerActions={
          Object.keys(formData).length > 0 && (
             <div className="menu-mgmt-actions">
                {formData.id && (
                  <button className="btn btn-outline menu-mgmt-delete-btn" onClick={handleDelete} title={t('common.delete', '삭제')} aria-label={t('common.delete', '삭제')}><Eraser size={16} /> {t('common.delete', '삭제')}</button>
                )}
          ...
          {Object.keys(formData).length > 0 ? (
            <div className="menu-mgmt-form-container">
              ...
              <div className="menu-mgmt-form-group">
                ...
                <input type="text" className="form-input" name="menuKey" value={formData.menuKey || ''} onChange={handleChange} title={t('menu.title_key', '메뉴 키')} placeholder="sidebar.menu_name" />
              </div>
              ...
              <div className="menu-mgmt-form-row">
                <div className="menu-mgmt-form-group menu-mgmt-form-col">
                  <label className="form-label">{t('menu.device_exposure', '기기 노출 설정')}</label>
                  <div className="menu-mgmt-checkbox-group">
                    <label className="checkbox-label">
                      <input 
                        type="checkbox" 
                        name="isPc" 
                        checked={formData.isPc === 'Y'} 
                        onChange={(e) => setFormData(prev => ({ ...prev, isPc: e.target.checked ? 'Y' : 'N' }))}
                      />
                      PC
                    </label>
                    <label className="checkbox-label">
                      <input 
                        type="checkbox" 
                        name="isMobile" 
                        checked={formData.isMobile === 'Y'} 
                        onChange={(e) => setFormData(prev => ({ ...prev, isMobile: e.target.checked ? 'Y' : 'N' }))}
                      />
                      Mobile
                    </label>
                  </div>
