...
import { Search, Plus, Download, Edit2, Save } from 'lucide-react';
    ...
    },
    {
      ...
      accessor: (row) => (
        <div className="flex gap-8">
          ...
          <button className="pagination-btn" title={t('common.delete')} onClick={(e) => { e.stopPropagation(); handleDelete(row.id, row.name); }}><Eraser size={16} color="var(--status-danger)" /></button>
        </div>
      ),
      colWidth: '120px',
      align: 'center'
        ...
        <Card title={t('user.filter.title')} collapsible>
          <div className="filter-panel horizontal grid-5">
            ...
            <InputField 
              ...
              fullWidth={false}
            />
            <button className="btn btn-primary self-end" onClick={handleFilter}><Search size={18} /> {t('user.filter.apply')}</button>
          </div>
        </Card>
